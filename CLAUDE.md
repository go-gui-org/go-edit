...
- `edit/buffer/` — document model. `Buffer` is `[]*line`, where `line` wraps `[]byte` (see `edit/buffer/line.go` — the type is the planned gap-buffer swap point). The single mutation choke point is `Buffer.Apply(Edit) Change`. `Load(io.Reader)` caps at `MaxLoadBytes` (32 MiB); `MaxLineBytes` (1 MiB) bounds per-line size. Extension substrate: `EditFilter` chain, `PostEditFunc` observers, `MarkSet` tracker, `DecorationProvider` interface.
...
Phases 0, 1, 1.2 (File I/O), 1.5 (Extension substrate — folds Phase 4 syntax highlighting), 2 (Selection + clipboard), 3 (Undo/redo), 5 (Multi-cursor), 6 (Search/replace), 7 (Quality of life: bracket match, wrap, folds, whitespace viz, sticky scroll), and 8 (Polish: per-language config, diagnostics gutter, help overlay) are committed. Search state lives in `editorState.Search`; input routing checks `Search.Active` before normal keymap dispatch. Golden-frame tests are deferred until an upstream `DrawContext.Inspect()` accessor exists. The per-line `[]byte` store (wrapped in an unexported `line` struct) will be replaced with a per-line gap buffer once bench pressure justifies it. See `ROADMAP.md` for the full phase list and open questions.
