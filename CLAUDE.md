...
Phases 0, 1, 1.2 (File I/O), 1.5 (Extension substrate), 2 (Selection + clipboard), and 3 (Undo/redo) are committed. Phase 3 adds linear undo/redo with typing coalescing, compound edit grouping (BeginGroup/EndGroup), cursor restore, and dirty-flag tracking through the undo stack. Golden-frame tests are deferred until an upstream `DrawContext.Inspect()` accessor exists. The plain-`[]byte` line store will be replaced with a per-line gap buffer once bench pressure justifies it. See `ROADMAP.md` for the full phase list and open questions.
