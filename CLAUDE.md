...
Phases 0, 1, 1.2 (File I/O), 1.5 (Extension substrate), and 2 (Selection + clipboard) are committed. Phase 2 adds shift-arrow and mouse selection (click/drag/double/triple), cut/copy/paste via go-gui clipboard, tab/shift-tab indent (selection-aware), and auto-indent on Enter. Golden-frame tests are deferred until an upstream `DrawContext.Inspect()` accessor exists. The plain-`[]byte` line store will be replaced with a per-line gap buffer once bench pressure from Phase 3 (undo) justifies it. See `ROADMAP.md` for the full phase list and open questions.
