...
func TriggerAction(w *gui.Window, focusID string, actionID string) {
	if w == nil || actionID == "" {
		return
	}
...
func CursorPos(w *gui.Window, focusID string) (line, col int, ok bool) {
	if w == nil {
		return 0, 0, false
	}
	sm := gui.StateMapRead[string, editorState](w, nsEdit)
	if sm == nil {
		return 0, 0, false
	}
